...
-- Prefix with 2x if ingredient appears twice
-- [Section logic reused: base_items, split_exclusions, split_extras, exclusion_names, extra_names, etc.]
...
--SELECT * FROM toppings_count ORDER BY item_id ASC, count DESC;
SELECT tc.topping_name, SUM(tc.count) as quantity_used 
FROM base_items bi JOIN toppings_count tc ON bi.item_id = tc.item_id
